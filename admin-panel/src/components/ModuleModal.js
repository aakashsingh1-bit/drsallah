    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white border border-[#e8e6e0] rounded-2xl w-full max-w-md shadow-card-lg animate-scale-in">
        <div className="flex items-center justify-between p-6 border-b border-[#e8e6e0]">
          <h2 className="text-lg font-bold text-[#1c1d1f]">{module ? 'Edit Module' : 'New Module'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <input className="field-input" required value={form.title} onChange={e => setForm({...form, title: e.target.value})} placeholder="e.g. Introduction to Algebra" />
          </div>
          <div>
          ...
          <label className="flex items-center gap-3 cursor-pointer" onClick={() => setForm({...form, isPublished: !form.isPublished})}>
            ...
            <span className="text-[13px] text-[#6a6f73]">Publish module</span>
          </label>
